...
class SkpdController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('backend.skpd.index', compact('config', 'page_breadcrumbs'));
    }

    public function store(Request $request)
    {
          $validator = Validator::make($request->all(), [
            'name' => 'required|unique:skpd,name',
            'deskripsi' => 'required',
          ]);
    ...
    {
        $validator = Validator::make($request->all(), [
            'name' => 'required|unique:skpd,name,'. $id,
            'deskripsi' => 'required',
          ]);
    ...
    public function select2(Request $request)
    {
      ...
      $data = Skpd::where('name', 'LIKE', '%' . $request->q . '%')
        ->orderBy('name')
        ->skip($offset)
        ->take($resultCount)
      ...
      $count = Skpd::where('name', 'LIKE', '%' . $request->q . '%')
        ->get()
        ->count();
